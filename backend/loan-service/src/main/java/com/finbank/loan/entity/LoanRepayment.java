...
@Entity @Table(name="loan_repayments",indexes={@Index(name="idx_repay_loan",columnList="loanReference"),@Index(name="idx_repay_status",columnList="status")},uniqueConstraints=@UniqueConstraint(name="uk_repay_loan_installment",columnNames={"loanReference","installmentNumber"})) @Getter @Setter
