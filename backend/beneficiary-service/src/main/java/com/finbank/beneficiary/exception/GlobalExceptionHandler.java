...
import com.finbank.beneficiary.dto.ApiErrorResponse;
...
public class GlobalExceptionHandler {
    ...
    public ApiErrorResponse notFound(BeneficiaryNotFoundException ex, HttpServletRequest request) {
        return error(404, ex.getMessage(), request);
    }

    @ExceptionHandler(DuplicateBeneficiaryException.class)
    @ResponseStatus(HttpStatus.CONFLICT)
    public ApiErrorResponse duplicate(DuplicateBeneficiaryException ex, HttpServletRequest request) {
