...
public class TransactionServiceImpl implements TransactionService {
    ...
    public TransactionServiceImpl(BankTransactionRepository repository) {
    ...
    public TransactionResponse createTransaction(CreateTransactionRequest request) {
        ...
        transaction.setCustomerNumber(request.customerNumber().trim());
    ...
    private void validateTransaction(CreateTransactionRequest request) {
        ...
        if (request.type() == TransactionType.DEPOSIT && request.destinationAccountNumber() == null) {
            throw new InvalidTransactionException("Destination account is required for deposit");
        }

        if (request.type() == TransactionType.WITHDRAWAL && request.sourceAccountNumber() == null) {
            throw new InvalidTransactionException("Source account is required for withdrawal");
        }

        if (request.type() == TransactionType.TRANSFER &&
                (request.sourceAccountNumber() == null || request.destinationAccountNumber() == null)) {
            throw new InvalidTransactionException("Source and destination accounts are required for transfer");
        }

        if (request.type() == TransactionType.TRANSFER &&
                request.sourceAccountNumber().trim().equalsIgnoreCase(request.destinationAccountNumber().trim())) {
            throw new InvalidTransactionException("Source and destination accounts must be different");
        }
    }

    private String generateReference() {
        return "FT" + UUID.randomUUID().toString().replace("-", "").substring(0, 16).toUpperCase();
    }
    ...
    private TransactionResponse toResponse(BankTransaction t) {
        return new TransactionResponse(
                t.getTransactionReference(), t.getIdempotencyKey(), t.getCustomerNumber(),
                t.getSourceAccountNumber(), t.getDestinationAccountNumber(), t.getType(),
                t.getStatus(), t.getAmount(), t.getCurrency(), t.getDescription(),
